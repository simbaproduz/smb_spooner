...
- Navegacao de preview com `seta para cima/baixo` depois de clicar em um item da lista de spawn
- O `Yaw` pode ser ajustado ainda no preview antes do spawn, e o `E` preserva essa rotacao do preview
...
| `Right click` | deletar |
| `C/V` | rotacionar o preview atual ou a entidade selecionada |
| `B` | trocar eixo de rotacao (`Pitch`, `Roll`, `Yaw`) |
| `Q/Z/Setas esquerda/direita` | ajustar posicao |
| `Seta para cima/baixo` | depois que um preview fica ativo ao clicar num item, navegar pela lista atual do preview |
...
- preview e spawn de objeto com rotacao sincronizada, incluindo `Yaw` ainda no preview; tecla `3` zera rotacao
